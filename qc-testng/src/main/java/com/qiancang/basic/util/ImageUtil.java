    ...
    public static void imageCompare(String expectedImgPath, String actualImgPath,Boolean strictMacth,int... expPercent){
        ...
        try {
            ...
            if(sizefileInput == sizefileOutPut){
                ...
                ap = (sizefileInput-count)/(float)sizefileInput;
            }
        } catch (IOException e) {
            LogUtils.error("图片比对过程中出现异常",e);
        }
        ...
        //实际相似度
        String actualSimiliar = nf.format(ap);
        //预期相似度
        String expectedSimiliar = nf.format(ep);

        if(strictMacth){//严格匹配
            if ("100%".equals(actualSimiliar)){
                LogUtils.info("采用严格匹配截图,图片实际相似度为:"+actualSimiliar+",预期相似度100%,,匹配通过pass");
            }
           Assertion.verifyEquals( actualSimiliar,"100%" ,"采用严格匹配截图,图片实际相似度为:"+actualSimiliar+",预期相似度100%,,匹配不通过failed");
        }else {//非严格匹配
            if (ap>=ep){
                LogUtils.info("采用非严格匹配截图,图片实际相似度为:"+actualSimiliar+",高于预定义"+expectedSimiliar+",匹配通过pass");
            }
            Assertion.verifyTrue( ap>=ep,"采用非严格匹配截图,图片实际相似度为:"+actualSimiliar+",低于预定义"+expectedSimiliar+",匹配不通过failed");
        }
