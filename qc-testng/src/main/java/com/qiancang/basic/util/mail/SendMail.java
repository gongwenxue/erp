...
public class SendMail {
	public static boolean sendHtmlMail(MailInfo mailInfo) {
		...
		try {
			...
			// 设置HTML内容
//			messageBodyPart.setContent(mailInfo.getContent(), "text/html; charset=gb2312");
...
//            LogUtils.info("erp自动化测试报告邮件发送成功");
			Transport.send(mailMessage);// 发送邮件
			return true;
		} catch (Exception e) {
			e.printStackTrace();
//			LogUtils.error("erp自动化测试报告邮件发送失败");
		}
		return false;
	}
